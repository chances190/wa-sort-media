...
def main(argv: list[str] | None = None) -> int:
    ...
    cg.add_argument("--contacts", help="Path to VCF contacts export")
    ...
    out_dir = Path(args.out) if args.out else Path(str(args.media) + "_organized")
    ...
    if args.wa_db:
        ...
        try:
            contacts = load_vcf_contacts(Path(args.contacts))
            logger.debug("Loaded contacts from file: %s", args.contacts)
        except (FileNotFoundError, ValueError) as e:
    ...
    logger.info("Done. %s files organized into %s/", count, out_dir)
    return 0
